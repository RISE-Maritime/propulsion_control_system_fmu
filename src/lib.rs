...
pub struct PropulsionControlSystem {
    #[input]
    pub minimum_consumption: f64, // t/24h
    pub setpoint_consumption: f64, // t/24h
    pub setpoint_speed: f64,       // knots
    pub actual_consumption: f64,   // t/24h
    pub actual_speed: f64,         // knots
...
impl FmuFunctions for PropulsionControlSystem {
    fn do_step(&mut self, _current_time: f64, _time_step: f64) {
        ...
        self.actual_consumption_relative_minimum_consumption =
            calculate_relative_value(self.actual_consumption, self.minimum_consumption);
        self.actual_consumption_relative_setpoint_consumption =
            calculate_relative_value(self.actual_consumption, self.setpoint_consumption);
        self.actual_speed_relative_setpoint_speed =
            calculate_relative_value(self.actual_speed, self.setpoint_speed);
        ...
        {
            self.lever = self.lever + self.actual_consumption_relative_minimum_consumption;
            self.mode = 0;
        } else if self.actual_consumption_relative_setpoint_consumption
            < self.actual_speed_relative_setpoint_speed
        {
            self.lever = self.lever + self.actual_consumption_relative_setpoint_consumption;
            self.mode = 1;
        } else {
            self.lever = self.lever + self.actual_speed_relative_setpoint_speed;
            self.mode = 2;
        }

        self.lever = f64::max(0.0, f64::min(1.0, self.lever));
    }
}
...
mod tests {
    ...
    fn test_mode_0_minimum_consumption() {
        let mut sys = PropulsionControlSystem {
            minimum_consumption: 10.0,
            setpoint_consumption: 20.0,
            setpoint_speed: 10.0,
    ...
    fn test_mode_priority() {
        let mut sys = PropulsionControlSystem {
            minimum_consumption: 5.0,
            setpoint_consumption: 10.0,
            setpoint_speed: 10.0,
            actual_consumption: 9.0,
            actual_speed: 11.0,
            lever: 0.5,
            ..Default::default()
        };
